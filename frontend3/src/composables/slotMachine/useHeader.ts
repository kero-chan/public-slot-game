...
/**
 * Use header interface
 */
export interface UseHeader {
  draw: (ctx: CanvasRenderingContext2D | null, rect: HeaderRect) => void
}

export function useHeader(canvasState: CanvasState, gameState: GameState): UseHeader {
  function draw(ctx: CanvasRenderingContext2D | null, rect: HeaderRect): void {
    if (!ctx) return
    const { w, h } = rect
